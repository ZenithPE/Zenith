...
abstract class BaseFlowerBed extends Flowable implements HorizontalFacing{
	...
	public function canBePlacedAt(Block $blockReplace, Vector3 $clickVector, int $face, bool $isClickedBlock) : bool{
		return ($blockReplace instanceof self && $blockReplace->hasSameTypeId($this) && $blockReplace->count < self::MAX_COUNT) || $this->supportedWhenPlacedAt($blockReplace, $clickVector, $face, $isClickedBlock);
	}

	public function place(BlockTransaction $tx, Item $item, Block $blockReplace, Block $blockClicked, int $face, Vector3 $clickVector, ?Player $player = null) : bool{
		if($blockReplace instanceof self && $blockReplace->hasSameTypeId($this) && $blockReplace->count < self::MAX_COUNT){
